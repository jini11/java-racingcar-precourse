...
### 1. 자동차 이름을 입력받는다.
  ...
  - [x] 이름은 1자 이상 5자 이하이다.
  - [x] 이름은 쉼표로 구분한다.
  - [x] [예외] 위 조건을 벗어날 경우 예외처리하고, 다시 입력받는다.
...
### 3. n번 동안 4~5번 과정을 반복한다.

### 4. 각 자동차를 전진하거나 멈춘다.
  - [x] 전진: 0~9 사이 랜덤 값을 구한 후 4 이상일 경우
  - [x] 멈춤: 0~9 사이 랜덤 값을 구한 후 4 미만일 경우

### 5. 라운드 결과를 출력한다.
  - [x] 자동차 이름과 움직인 거리를 `-`로 표시한다.

### 6. 최종 우승자를 출력한다.
  - [x] 가장 먼 거리를 간 자동차가 우승한다.
  - [x] 우승자는 1명 이상일 수 있다.
  - [x] 각 우승자는 쉼표로 구분한다.
